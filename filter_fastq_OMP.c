...
int main(int argc,char **argv) {
  ...
  #pragma omp parallel
  {
    ...
    int stop;
    if(rank == size-1) {
      // add the extrra times that last thread has to run
    ...
    for(line_count = 0; line_count < stop; line_count++) {
      ...
      //write the filtered fastq to the temp file
  ...
  // concat files to out.fastq and then remove temp files
  system("cat out_*.fastq > out.fastq");
  system("rm out_*.fastq");
